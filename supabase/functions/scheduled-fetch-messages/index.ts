...
serve(async (req) => {
  ...
  try {
    ...
    // Get ALL active source channels across all users
    ...
    if (!channels || channels.length === 0) {
      return new Response(JSON.stringify({ message: "No active source channels", count: 0 }), {
    ...
    console.log(`Scheduled fetch complete: ${totalFetched} messages from ${channels.length} channels`);

    return new Response(JSON.stringify({
      message: `${totalFetched} messages fetched`,
      count: totalFetched,
    ...
    console.error("scheduled-fetch-messages error:", e);
